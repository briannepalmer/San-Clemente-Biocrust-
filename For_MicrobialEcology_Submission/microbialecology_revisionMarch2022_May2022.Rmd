...
```{r}
taxaNMDSRevisedWild <- ggplot(NMDS %>% filter(Fire.Type %in% c("control2012", "wildfire")), aes(x = MDS1, y = MDS2, col = Fire.Type)) + geom_point(size = 5) + 
...
taxaNMDSRevisedRX <- ggplot(NMDS %>% filter(Fire.Type %in% c("control2017", "prescribed")), aes(x = MDS1, y = MDS2, col = Fire.Type)) + geom_point(size = 5) + 
  stat_ellipse(size = 3) + theme_bw() + theme(text=element_text(size=30))  + theme(legend.position = "bottom") +
    scale_color_manual(values = c("dodgerblue4","gold"))

#include in caption r2 = 0.14. p=0.334, stress = 0.096

taxaNMDSRevisedRX

library(patchwork)
pdf("C:/Users/Brianne/OneDrive - San Diego State University (SDSU.EDU)/San-Clemente-Biocrust-/figures/taxaNMDSRevised.pdf", height = 12, width = 20)
taxaNMDSRevisedRX + taxaNMDSRevisedWild 
dev.off()
```
...
```{r}
richness.plotRX <- ggplot(data = rich.div %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = FireType, y = richness, fill = FireType)) + geom_boxplot() + theme_bw() + theme(legend.position = "none") + scale_fill_manual(values = c("dodgerblue4", "gold")) + theme(text=element_text(size=30)) + labs(y = "# of Genera") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +ylim(700,850)

richness.plotWild <- ggplot(data = rich.div %>% filter(FireType %in% c("wildfire", "control2012")), aes(x = FireType, y = richness, fill = FireType)) + geom_boxplot() + theme_bw() + theme(legend.position = "none") + scale_fill_manual(values = c("forestgreen", "salmon")) + theme(text=element_text(size=30)) + labs(y = "# of Genera") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + ylim(700,850)

```

```{r}
pdf("figures/richness.pdf")
richness.plotRX + richness.plotWild
dev.off()
```

```{r}
diversity.plotRX <- ggplot(data = rich.div %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = FireType, y = diversity, fill = FireType)) + geom_boxplot() + theme_bw() + theme(legend.position = "none") + scale_fill_manual(values = c("dodgerblue4", "gold")) + theme(text=element_text(size=30)) + labs(y = "Shannon-Weiner Index") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+ ylim(4.5,5.5)

diversity.plotWild <- ggplot(data = rich.div %>% filter(FireType %in% c("wildfire", "control2012")), aes(x = FireType, y = diversity, fill = FireType)) + geom_boxplot() + theme_bw() + theme(legend.position = "none") + scale_fill_manual(values = c("forestgreen", "salmon")) + theme(text=element_text(size=30)) + labs(y = "Shannon-Weiner Index") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + ylim(4.5,5.5)
```

```{r}
pdf("figures/diversity.pdf")
diversity.plotRX + diversity.plotWild
dev.off()
```
...
```{r}
chl.mod <- lmer(chl_total_mg_per_g ~ FireType + year + (1|ID), data = chl %>% filter(FireType %in% c("control2017", "prescribed")))
residplot(chl.mod)
Anova(chl.mod)
...
eps.mod <- lmer(Conc_mg_g ~ FireType + (1|BSC_ID), data = eps %>% filter(FireType %in% c("control2012", "wildfire")))
residplot(eps.mod)
Anova(eps.mod)
...
```{r}
nfix.mod <- glm(log(BNF) ~ FireType, data = nfix.no.out %>% filter(FireType %in% c("control2012" ,"wildfire")))
residplot(nfix.mod)
...
```{r}
chlplotRX <- ggplot(data = chl %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = FireType, y = chl_total_mg_per_g, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold")) + xlab("Fire Type") + ylab("Total Chlorophyll (mg/g)") + theme(legend.position = "none") + theme(text=element_text(size=20))+ labs(x = "Fire Type")+  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

chlplotWild <- ggplot(data = chl %>% filter(FireType %in% c("wildfire", "control2012")), aes(x = FireType, y = chl_total_mg_per_g, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c( "forestgreen", "salmon")) + xlab("Fire Type") + ylab("Total Chlorophyll (mg/g)") + theme(legend.position = "none") + theme(text=element_text(size=20))+ labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))


```

```{r}
eps.plotRX <- ggplot(data = eps %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = FireType, y = Conc_mg_g, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold")) + xlab("Fire Type") + ylab("Total EPS (mg/g)") + theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

eps.plotWild <- ggplot(data = eps %>% filter(FireType %in% c("wildfire", "control2012")), aes(x = FireType, y = Conc_mg_g, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("forestgreen", "salmon")) + xlab("Fire Type") + ylab("Total EPS (mg/g)") + theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

```

```{r}
nfix.plotRX <- ggplot(data = nfix %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = FireType, y = BNF, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold")) + xlab("Fire Type") + ylab("nmol N Uptake/mg *DW/ hour") + theme(legend.position = "none")+ theme(text=element_text(size=20)) + labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

nfix.plotWild <- ggplot(data = nfix %>% filter(FireType %in% c("wildfire", "control2012")), aes(x = FireType, y = BNF, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("forestgreen", "salmon")) + xlab("Fire Type") + ylab("nmol N Uptake/mg *DW/ hour") + theme(legend.position = "none")+ theme(text=element_text(size=20)) + labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
function.nmdsRX <- ggplot(NMDS %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = MDS1, y = MDS2, col = FireType)) + geom_point() + 
  stat_ellipse(size = 3) + theme_bw() + scale_color_manual(values = c("dodgerblue4", "gold"))  + theme(text=element_text(size=30)) + theme(legend.position = "bottom")

function.nmdsWild <- ggplot(NMDS %>% filter(FireType %in% c("wildfire", "control2012")), aes(x = MDS1, y = MDS2, col = FireType)) + geom_point() + 
  stat_ellipse(size = 3) + theme_bw() + scale_color_manual(values = c("forestgreen", "salmon"))  + theme(text=element_text(size=30)) + theme(legend.position = "bottom")

#annotate("text", x=0.1, y=0.20, label= "R2 = 0.15", size = 10) + annotate("text", x=0.1, y=0.18, label= "P = 0.244", size = 10) + annotate("text", x=0.1, y=0.16, label= "stress = 0.093", size = 10) + theme(text=element_text(size=30)) 


pdf("figures/functionNMDSrevised.pdf", height = 12, width = 20)
function.nmdsRX + function.nmdsWild
dev.off()
...
```{r}
pdf("figures/figure4.pdf", width = 15, height = 30)
(richness.plotRX + richness.plotWild)/(diversity.plotRX + diversity.plotWild)/(taxaNMDSRevisedRX + taxaNMDSRevisedWild)/(function.nmdsRX + function.nmdsWild)
dev.off()
```
...
ggplot(data = sub_df, aes(x = name, y = value, fill = FireType))+ 
  geom_bar(stat = "identity", position = "dodge") + 
  theme_bw() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + 
  scale_fill_manual(values = c( "forestgreen","dodgerblue", "gold", "salmon"))  + 
...
```{r}
n.fix <- glm(Nitrogen_fixation ~ FireType, data = bsc.level3.all %>% filter(FireType %in% c("control2012", "wildfire")))
Anova(n.fix)
plot(n.fix)
...
```{r}
eps <- glm(Exopolysaccharide_Biosynthesis ~ FireType, data = bsc.level3.all %>% filter(FireType %in% c("prescribed", "control2017")))
Anova(eps)
plot(eps)
...
```{r}
chl <- glm(Chlorophyll_Biosynthesis ~ FireType, data = bsc.level3.all %>% filter(FireType %in% c("wildfire", "control2012")))
Anova(chl)
plot(chl)
...
eps.genesRX <- ggplot(bsc.level3.all %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = FireType, y = Exopolysaccharide_Biosynthesis, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold"))+ theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

eps.genesWild <- ggplot(bsc.level3.all%>% filter(FireType %in% c("wildfire", "control2012")), aes(x = FireType , y = Exopolysaccharide_Biosynthesis, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c( "forestgreen","salmon"))+ theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

```

```{r}
nfix.genesRX <- ggplot(bsc.level3.all %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = FireType, y = Nitrogen_fixation, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold"))+ theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

nfix.genesWild <- ggplot(bsc.level3.all %>% filter(FireType %in% c("wildfire", "control2012")), aes(x = FireType, y = Nitrogen_fixation, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("forestgreen","salmon"))+ theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

```

```{r}
chl.genesRX <- ggplot(bsc.level3.all %>% filter(FireType %in% c("prescribed", "control2017")), aes(x = FireType, y = Chlorophyll_Biosynthesis, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4","gold"))+ theme(legend.position = "none")+ theme(text=element_text(size=20)) + labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

chl.genesWild <- ggplot(bsc.level3.all%>% filter(FireType %in% c("wildfire", "control2012")), aes(x = FireType, y = Chlorophyll_Biosynthesis, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("forestgreen", "salmon"))+ theme(legend.position = "none")+ theme(text=element_text(size=20)) + labs(x = "Fire Type")+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

```

```{r}
pdf("figures/allfunctions.pdf", height = 25, width = 25)
(chlplotRX  + chlplotWild + chl.genesRX+ chl.genesWild)/(eps.plotRX +eps.plotWild + eps.genesRX + eps.genesWild)/(nfix.plotRX +nfix.plotWild + nfix.genesRX+ nfix.genesWild)
dev.off()
