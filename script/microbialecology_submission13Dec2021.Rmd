...
bsc_taxa$FireType <- c("prescribed","prescribed","prescribed","prescribed","control", "control", "control", "control","prescribed","prescribed","prescribed","prescribed","control", "control", "control", "control", "wildfire", "wildfire", "wildfire", "wildfire", "control", "control", "control", "control")


# filter and remove metadata 
rx <- (bsc_taxa %>% filter(FireType == "prescribed"))[-c(1:90)]
wild <- (bsc_taxa %>% filter(FireType == "wildfire"))[-c(1:90)]
cont <- (bsc_taxa %>% filter(FireType == "control"))[-c(1:90)]

rx <- rx %>% mutate_all(as.numeric)
wild <- wild %>% mutate_all(as.numeric)
cont <- cont %>% mutate_all(as.numeric)

rx[is.na(rx)] <- 0
wild[is.na(wild)] <- 0
cont[is.na(cont)] <- 0

rx.rich <- specnumber(rx)
wild.rich <- specnumber(wild)
cont.rich <- specnumber(cont)
rx.div <- vegan::diversity(rx)
wild.div <- vegan::diversity(wild)
cont.div <- vegan::diversity(cont)

# make a vector for richness and diversity 

richness <- c(rx.rich, wild.rich, cont.rich)
diversity <- c(rx.div, wild.div, cont.div)
...
rich.lm <- lm(richness ~ FireType*Site, data = rich.div)
Anova(rich.lm)
x = residuals(rich.lm)
...
div.lm <- lm(diversity ~ treatment* site, data = rich.div)
Anova(div.lm, type = "II")
x = residuals(div.lm)
plotNormalHistogram(x)
plot(fitted(div.lm),
     residuals(div.lm))
...
richness.plot <- ggplot(data = rich.div, aes(x = FireType, y = richness, fill = FireType)) + geom_boxplot() + theme_bw() + theme(legend.position = "none") + scale_fill_manual(values = c("dodgerblue4", "gold", "salmon")) + theme(text=element_text(size=30))
dev.off()
...
diversity.plot <- ggplot(data = rich.div, aes(x = FireType, y = diversity, fill = FireType)) + geom_boxplot() + theme_bw() + theme(legend.position = "none") + scale_fill_manual(values = c("dodgerblue4", "gold", "salmon")) + theme(text=element_text(size=30))
dev.off()
...
chl.mod <- lmer(chl_total_mg_per_g ~ site + fire.type + year + (1|ID), data = chl)
residplot(chl.mod)
Anova(chl.mod)
...
nfix.mod <- glm(log(BNF) ~ Site*Treatment*Year, data = nfix.no.out)
residplot(nfix.mod)
...
```{r}
pdf("C:/Users/Brianne/OneDrive - San Diego State University (SDSU.EDU)/San-Clemente-Biocrust-/figures/chlorophyllPlot.pdf")
chlplot <- ggplot(data = chl, aes(x = fire.type, y = chl_total_mg_per_g, fill = fire.type)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold", "salmon")) + xlab("Fire Type") + ylab("Total Chlorophyll (mg/g)") + theme(legend.position = "none") + theme(text=element_text(size=20))+ labs(x = "Fire Type")
dev.off()

```

```{r}
pdf("figures/EPSPlot.pdf")
eps.plot <- ggplot(data = eps, aes(x = FireType, y = Conc_mg_g, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold", "salmon")) + xlab("Fire Type") + ylab("Total EPS (mg/g)") + theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")
dev.off()

```

```{r}
pdf("figures/NFixPlot.pdf")
nfix.plot <- ggplot(data = nfix, aes(x = FireType, y = BNF, fill = FireType)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold", "salmon")) + xlab("Fire Type") + ylab("nmol N Uptake/mg *DW/ hour") + theme(legend.position = "none")+ theme(text=element_text(size=20)) + labs(x = "Fire Type")
dev.off()
...
metadata$FireType <- as.factor(metadata$FireType)
...
pdf("figures/functionNMDS.pdf")
function.nmds <- 
  ggplot(NMDS, aes(x = MDS1, y = MDS2, col = FireType)) + geom_point() + 
  stat_ellipse() + theme_bw() + scale_color_manual(values = c("dodgerblue4", "gold", "salmon"))  + theme(text=element_text(size=30)) + theme(legend.position = "bottom")
dev.off()
...
(richness.plot + diversity.plot)/(taxanmds + function.nmds)
dev.off()
```


```{r}
bsc.level3.all$fire.type<- c("prescribed","prescribed","prescribed","prescribed","control", "control", "control", "control","prescribed","prescribed","prescribed","prescribed","control", "control", "control", "control", "wildfire", "wildfire", "wildfire", "wildfire", "control", "control", "control", "control")
bsc.level3.all$fire.type <- as.factor(bsc.level3.all$fire.type)

n.fix <- glm(Nitrogen_fixation ~ fire.type, data = bsc.level3.all)
Anova(n.fix)
plot(n.fix)
...
```{r}
eps <- glm(Exopolysaccharide_Biosynthesis ~ fire.type, data = bsc.level3.all)
Anova(eps)
plot(eps)
summary(glht(eps, linfct = mcp(fire.type = "Tukey")))
```

```{r}
chl <- glm(Chlorophyll_Biosynthesis ~ fire.type, data = bsc.level3.all)
Anova(chl)
plot(chl)
summary(glht(chl, linfct = mcp(fire.type = "Tukey")))
```

```{r}
pdf("figures/EPSGenes.pdf")
eps.genes <- ggplot(bsc.level3.all, aes(x = fire.type, y = Exopolysaccharide_Biosynthesis, fill = fire.type)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold", "salmon"))+ theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")
dev.off()

```

```{r}
pdf("figures/NfixGenes.pdf")
nfix.genes <- ggplot(bsc.level3.all, aes(x = fire.type, y = Nitrogen_fixation, fill = fire.type)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold", "salmon"))+ theme(legend.position = "none")+ theme(text=element_text(size=20))+ labs(x = "Fire Type")
dev.off()

```

```{r}
pdf("figures/ChlGenes.pdf")
chl.genes <- ggplot(bsc.level3.all, aes(x = fire.type, y = Chlorophyll_Biosynthesis, fill = fire.type)) + geom_boxplot() + theme_bw() + scale_fill_manual(values = c("dodgerblue4", "gold", "salmon"))+ theme(legend.position = "none")+ theme(text=element_text(size=20)) + labs(x = "Fire Type")
dev.off()
...
meta <- meta[c(1,2,3,5)]
meta$fire.type <- c("rx", "rx", "rx", "rx", "cont", "cont", "cont", "cont", "rx", "rx", "rx", "rx", "cont", "cont", "cont", "cont", "wild", "wild", "wild", "wild", "cont", "cont", "cont", "cont")

meta <- meta %>% filter(fire.type == "cont")

sampleData <- sample_data(meta)

sample_names(sampleData) <- c("bsc5", "bsc6", "bsc7", "bsc8", "bsc13", "bsc14", "bsc15", "bsc16", "bsc21", "bsc22", "bsc23", "bsc24")
```
...
taxon <- taxon[c(1:7, 12:15, 20:23, 28:31)] # keep only control samples
taxon$sum <- rowSums(taxon[,c(8:19)])

taxon$sum <- as.vector(taxon$sum)
taxon[with(taxon, order("sum"))]
# use only the top 500 most abundant 

taxon500 <- taxon[c(1:500),]
#taxon <- taxon %>% filter(domain != "Eukaryota")
otu_table <- taxon500 %>%  dplyr::select(otu, bsc5:bsc24)
...
pdf("figures/control_network.pdf")
...
pdf("figures/network analysis.order.control.pdf")
set.seed(200)
...
pdf("figures/cyano_network.pdf",  width = 18, height = 18)
control.cyano.network <- chordDiagram(phylum.df, annotationTrack = "grid", annotationTrackHeight = 0.01, preAllocateTracks = 1, link.visible = phylum.df[3] > 0.75, grid.col = c("cadetblue", "darkolivegreen3", "darkslateblue", "lightcoral", "mediumpurple1", "salmon1", "tomato", "goldenrod", "darkslategray","darkgray", "indianred3", "lightskyblue3", "plum4"))
for(si in get.all.sector.index()) {
    xlim = get.cell.meta.data("xlim", sector.index = si, track.index = 1)
    ylim = get.cell.meta.data("ylim", sector.index = si, track.index = 1)
    circos.text(mean(xlim), ylim[1], si, facing = "clockwise", adj = c(-0.1, -0.2),
...
sample_names(sampleData) <- c("bsc1", "bsc2", "bsc3", "bsc4", "bsc9", "bsc10", "bsc11", "bsc12", "bsc17", "bsc18", "bsc19", "bsc20")
```
...
taxon <- taxon[c(1:11, 16:19, 24:27)] # keep only burned samples
taxon$sum <- rowSums(taxon[,c(8:19)])
...
taxon500 <- taxon[c(1:500),]
#taxon <- taxon %>% filter(domain != "Eukaryota")
otu_table <- taxon500 %>%  dplyr::select(otu, bsc1:bsc20)
