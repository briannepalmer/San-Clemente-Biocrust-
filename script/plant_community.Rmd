...
plants <- plants %>% filter(Year == 2018 | Year == 2019, T_Year == 2017) 
...
plants.short <- plants.short %>% rename(ID = ï..Ref)
...
ggplot(data=NMDS, aes(x = MDS1, y = MDS2)) +
  geom_point(data=NMDS, aes(MDS1, MDS2, shape =year),position=position_jitter(.1)) + stat_ellipse(aes(fill=treatment), alpha=.2,type='t',size =1, geom="polygon")+ 
