...
bestmod.log <- lmer(Cover ~ Treatment + Site + (1|Plot), data = data %>% filter(Funct.Gp == "nf", Year == "2019"), REML=FALSE)
summary(bestmod.log)
Anova(bestmod.log)
...
bestmod.log <- lmer(Cover ~ Treatment + Site + (1|Plot), data = data %>% filter(Funct.Gp == "nnf", Year == "2019"), REML=FALSE)
summary(bestmod.log)
Anova(bestmod.log)
...
df2 <- summarySE(data, measurevar = "Cover", groupvars=c("Treatment", "Year"))
head(df2)

df2$Year <- as.numeric(df2$Year)

pdf("figures/plantcover.pdf")
ggplot(data = df2, aes(x = Treatment, y = Cover, fill = Treatment)) +
  geom_bar(sta = "identity") + 
  geom_errorbar(aes(ymin=(Cover), ymax=(Cover+sd)), width=.2,
                 position=position_dodge(0.05)) + theme_bw()+ facet_wrap(.~ as.factor(Year)) + ggtitle("All Plant Cover")

dev.off()


df2 <- summarySE(data %>% filter(Funct.Gp == "ng"), measurevar = "Cover", groupvars=c("Treatment", "Year"))
ggplot(data = df2, aes(x = Treatment, y = Cover, fill = Treatment)) +
  geom_bar(sta = "identity") + 
  geom_errorbar(aes(ymin=(Cover-se), ymax=(Cover+se)), width=.2,
                 position=position_dodge(0.05)) + theme_bw()+ facet_wrap(.~ as.factor(Year))+ ggtitle("Native Grass Cover")

df2 <- summarySE(data %>% filter(Funct.Gp == "nng"), measurevar = "Cover", groupvars=c("Treatment", "Year"))
ggplot(data = df2, aes(x = Treatment, y = Cover, fill = Treatment)) +
  geom_bar(sta = "identity") + 
  geom_errorbar(aes(ymin=(Cover-se), ymax=(Cover+se)), width=.2,
                 position=position_dodge(0.05)) + theme_bw()+ facet_wrap(.~ as.factor(Year))+ ggtitle("NonNative Grass Cover")

df2 <- summarySE(data %>% filter(Funct.Gp == "nnf"), measurevar = "Cover", groupvars=c("Treatment", "Year"))
ggplot(data = df2, aes(x = Treatment, y = Cover, fill = Treatment)) +
  geom_bar(sta = "identity") + 
  geom_errorbar(aes(ymin=(Cover-se), ymax=(Cover+se)), width=.2,
                 position=position_dodge(0.05)) + theme_bw()+ facet_wrap(.~ as.factor(Year))+ ggtitle("NonNative Forb Cover")

df2 <- summarySE(data %>% filter(Funct.Gp == "nf"), measurevar="Cover", groupvars=c("Treatment", "Year"))
ggplot(data = df2, aes(x = Treatment, y = Cover, fill = Treatment)) +
  geom_bar(sta = "identity") + 
  geom_errorbar(aes(ymin=(Cover-se), ymax=(Cover+se)), width=.2,
                 position=position_dodge(0.05)) + theme_bw()+ facet_wrap(.~ as.factor(Year))+ ggtitle("Native Forb Cover")
