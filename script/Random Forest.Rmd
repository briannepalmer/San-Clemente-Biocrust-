...
print(rf)
varImpPlot(rf)
predicion_for_table <- predict(rf, test[,-9])
table(observed = test[,8], predicted = predicion_for_table)
...
ggplot(macro_plant, aes(x = Total_log, y = total_plant, color = Treatment)) + geom_point() + geom_smooth(method = "glm") + facet_grid(.~Treatment)+ stat_cor()
