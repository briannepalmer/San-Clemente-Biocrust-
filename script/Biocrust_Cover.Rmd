...
```{r load data}
data <- read.csv("data/sci_bsc_18_19_20_editMarch2021.csv")
qqnorm(data$Total)
...
mossmod.log <- lmer(log(Moss +1) ~ Treatment + Sample.Year + Site+ (1|Plot), data = data %>% filter(Sample.Year != "2020"))
residplot(mossmod.log)
Anova(mossmod.log)
summary(glht(mossmod.log, linfct = mcp(Treatment = "Tukey")))
summary(glht(mossmod.log, linfct = mcp(Sample.Year = "Tukey")))
...
ggplot(df2 %>% filter(Sample.Year != "2020", name != "Mixed"), aes(x=Year, y=percent*100, group=Treatment, color=Treatment)) + 
  geom_line() + 
  geom_point()+ 
